...
old_posteriors = lapply(cogs, posterior_gen, 
                        grid = p_grid, prior = app_prior, data = y2_data,
                        give_sample = F)
...
tidy_posterior_plot(9, 10, 3, 6) +
  labs(title = "Riccardo", y = "probability mass (density for old_prior)")

tidy_posterior_plot(34, 65, 66, 132) +
  labs(title = "Mikkel", y = "probability mass (density for old_prior)")

tidy_posterior_plot(148, 172, 160, 198) +
  labs(title = "Josh", y = "probability mass (density for old_prior)")

tidy_posterior_plot(8, 12, 2, 4) +
  labs(title = "Kristian", y = "probability mass (density for old_prior)")
...
two_posteriors <- function(correct, n, 
  ...
  return(cog %>%
    select(new_posterior = posterior, old_posterior, p_grid) %>%
    gather(key, value, -p_grid) %>% 
    mutate(key = factor(key, levels = 
                          c("old_posterior", "new_posterior"))) %>%
    ggplot(aes(x = p_grid, ymin = 0, ymax = value, fill = key)) +
      geom_ribbon(alpha = 0.5) +
      theme_janco_point() +
      labs(title = paste0(cog_name),
           subtitle = "comparing posterior distributions",
           y = "probability mass")
    )
  }
```
